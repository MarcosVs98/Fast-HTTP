...
class HTTPBoost():
	...
	def __init__(self, concurrent_requests, max_queue_size=0, concurrent_blocks=None, **kwargs):
		...
		self.queue_block         = asyncio.Queue(maxsize=self.max_queue_size)
		self.queue_result        = asyncio.Queue(maxsize=self.max_queue_size)
	...
	async def recover_block(self, bl):

		# recover block a urls and send it to a quick response

		for c, url in enumerate(self.urls, bl):
			try:
				self.kwargs['url'] = url + f'&count={c}' # GAMBI

				request  = HTTPClient()
				future   = asyncio.ensure_future(request.fetch(**self.kwargs))#, loop=self.loop)
				await self.queue_block.put(future)
				await self.rnd_sleep(.1)
	...
	async def quick_response(self):
		try:
			self.loop = self.get_event_loop()
			#asyncio.set_event_loop(self.loop)
			#self.loop.set_debug(True)

			self.loop.run_until_complete(asyncio.wait(self.queue_block.queue, return_when=asyncio.FIRST_COMPLETED))
			...
			while True:
				...
				task = await self.queue_block.get()
				#await self.rnd_sleep(.1)

				self.queue_block.task_done()

				if not task.cancelled():
					await self.queue_result.put(task.result())
	...
	async def run(self):
		...
		for nb in range(self.fake_block_size):
			...
			await self.recover_block(bl)
			bl = bl + len(self.urls)
			await self.quick_response()
...
def main():
	...
	# Teste assincrono
	assincrone_res = HTTPBoost(url='http://127.0.0.1:9999/api/?method=xpto.get&format=json', method='get', concurrent_requests=50)
	asyncio.run(assincrone_res.run())
